...
public class matrixOps {
    ...
    public static double[][] read_matrix(String encoded_matrix) {        
        ...
        for (int s=0;s<splitted.length;s++){
            if(s==0){
                rows=Integer.parseInt(splitted[s]);
                continue;
            }
            if(s==1){
                cols=Integer.parseInt(splitted[s]);
                continue;
            }
            matrix=new double[rows][cols];
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < cols; j++)
                    matrix[i][j] = Double.parseDouble(splitted[s]);
